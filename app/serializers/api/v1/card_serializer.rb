...
class Api::V1::CardSerializer < Api::V1::BaseSerializer
  attributes %i[id user product state activation_code created_at updated_at]
  ...
  def user
    "Api::V1::#{object.user.class}Serializer".constantize.new(object.user)
  end
  ...
  APPROVED_CARD = {
    cards: %i[id user product state activation_code created_at updated_at]
  }.freeze
end
